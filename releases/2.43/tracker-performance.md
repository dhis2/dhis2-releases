...
p95 values come from [`gstat` v0.2.0](https://github.com/dhis2/gatling-statistics/releases/tag/v0.2.0) over each run's `simulation.csv`; see [its percentile note](https://github.com/dhis2/gatling-statistics#percentiles) for why these will not exactly match the Gatling HTML report. All p95 values are in milliseconds. Throughput (req/s) and request counts are computed from `simulation.csv`.
